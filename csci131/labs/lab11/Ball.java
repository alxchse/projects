 ...
 *  Updated:  9/11/2021 - fixed several comments
 *  
...
public class Ball { 
    ...
    /**
     * Set the velocity of this ball (both speed and direction). Units are
     * "meters" per second. The velocity values affect how much the ball will
    ...
    public boolean touching(Star star) {
        // this ball is touching star if the distance
        // is less than our combined sizes.
    ...
    public boolean covering(Star star) {
        // this ball is covering star if we are bigger than
        // the distance plus their size.
    ...
    /**
     * Modify this ball's vertical velocity to simulate the effect of gravity
     * for the given duration (an amount of time, in milliseconds).
     */
    public void gravity(double duration) {
        vy -= duration * 9.8 / 10000;
    }

    /**
     * If this ball is touching any edge of the box ("wall"), this ball will
     * change it's direction so that it is moving directly away from that box
     * edge.
     */
    public void bounce(Box wall) {
        if (vy < 0 && wall.contains(x, y-radius))
            vy *= -1;
        else if (vy > 0 && wall.contains(x, y+radius))
            vy *= -1;
        if (vx < 0 && wall.contains(x-radius, y))
            vx *= -1;
        else if (vx > 0 && wall.contains(x+radius, y))
            vx *= -1;
    }
