...
template<class ItemType>
ArrayBag<ItemType>::ArrayBag(int capacity): itemCount(0), maxItems(capacity) {
  // The items array is already created, with size DEFAULT_CAPACITY, and we do
  // not yet support growing/modifying the array, so the requested capacity must
  // be no larger than DEFAULT_CAPACITY.
  assert(maxItems <= DEFAULT_CAPACITY);
