...
class BinaryNodeTree : public BinaryTreeInterface<ItemType> {
  ...
  protected:
    ...
    // Check if two subtrees have the same shape.
    template<class OtherItemType>
    bool isSameShapeHelper(const BinaryNode<ItemType>* subtree1,
  ...
  public:
    ...
    // Traverse using a 1-argument visit function, using a level-order traversal
    // strategy.
    void levelorderTraverse(void visit(ItemType)) const;

    // Check whether this tree and the other tree have the same "shape"
    // (ignoring the values in the nodes, but focusing only on the presence of
    // the nodes and how they are linked as left and right children).
    template<class OtherItemType>
    bool isSameShape(const BinaryNodeTree<OtherItemType>& other) const;
