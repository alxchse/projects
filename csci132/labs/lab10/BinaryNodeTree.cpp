...
template<class ItemType>
void BinaryNodeTree<ItemType>::swapSubTree() {
    swapSubTreeHelper(rootPtr);
}

template<class ItemType>
void BinaryNodeTree<ItemType>::swapSubTreeHelper(BinaryNode<ItemType>* subTree) {
  if (subTree == nullptr)
    return;
  else {
    auto left = subTree->getLeftChildPtr();
    auto right = subTree->getRightChildPtr();
    subTree->setLeftChildPtr(right);
    subTree->setRightChildPtr(left);
    swapSubTreeHelper(subTree->getLeftChildPtr());
    swapSubTreeHelper(subTree->getRightChildPtr());
  }
}

template<class ItemType>
ItemType BinaryNodeTree<ItemType>::getMin(const BinaryNode<ItemType>* subTree) const {
  if (subTree->isLeaf())
    return subTree->getItem();
  else {
    auto firstMin = subTree->getItem();
    if (subTree->getLeftChildPtr() != nullptr) {
      auto leftMin = getMin(subTree->getLeftChildPtr());
      firstMin = min(leftMin, firstMin);
    } if (subTree->getRightChildPtr() != nullptr) {
      auto rightMin = getMin(subTree->getRightChildPtr());
      firstMin = min(rightMin, firstMin);
    }
    return firstMin;
  }
}
...
template<class ItemType>
ItemType BinaryNodeTree<ItemType>::getMax(const BinaryNode<ItemType>* subTree) const {
  if (subTree->isLeaf())
    return subTree->getItem();
  else {
    auto firstMax = subTree->getItem();
    if (subTree->getLeftChildPtr() != nullptr) {
      auto leftMax = getMax(subTree->getLeftChildPtr());
      firstMax = max(leftMax, firstMax);
    } if (subTree->getRightChildPtr() != nullptr) {
      auto rightMax = getMax(subTree->getRightChildPtr());
      firstMax = max(rightMax, firstMax);
    }
    return firstMax;
  }
}
...
template<class ItemType>
bool BinaryNodeTree<ItemType>::isBSTHelper(const BinaryNode<ItemType>* subtree) const {
  if (subtree == nullptr)
    return true;
  auto left = subtree->getLeftChildPtr();
  auto right = subtree->getRightChildPtr();
  if (left != nullptr && subtree->getItem() <= getMax(left)) {
   return false; 
  }
  if (right != nullptr && subtree->getItem() >= getMin(right)) {
   return false; 
  }
  return isBSTHelper(left) && isBSTHelper(right);
